...
run()
{
    ...
    unshare -u -p --net -m -i --kill-child --mount-proc --root="$CONTAINERDIR/$NAME/rootfs" /bin/bash -c "mount -t proc proc /proc && mount -t sysfs sys /sys && chmod 755 / && $INIT_CMD_ARGS"
}
...
exec()
{
    ...
    nsenter -a -w -r --target $CONTAINER_INIT_PID /bin/bash -c "$EXEC_CMD_ARGS" 
...
addnetwork()
{
    ...
    ip link set $INSIDE_PEER netns $NAME up
